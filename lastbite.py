...
elif analysis_level == "Brand Company":
...
df['additional_order_value'] = df['additional_qty_pcs_increase'] * df['cogs']
...
df['additional_order_value_fmt'] = df['additional_order_value'].apply(lambda x: f"{int(x):,}" if x > 0 else "-")
...
elif analysis_level == "Brand Company" and submitted:
