...
# File Uploads
supply_file = st.sidebar.file_uploader("Upload Historical Supply SO (Exc. CANCELLED - until Today if all COMPLETED no more PICKING etc)", type=["xlsx"])
oos_file = st.sidebar.file_uploader("Upload Historical OOS%", type=["xlsx"])
...
if supply_file and oos_file:
    ...
    with st.expander("📌 Key Highlights of the OOS Projection"):
        st.markdown("""
        WILL BE MORE ACCURATE WITH MORE HISTORICAL DATA
        - **Note**: 31 Mar -> half day, Apr demand a bit lower than March
        - **Note STO**: 
