...
if supply_file and oos_file:
   ...
   # Ensure the data is sorted
    ...
    for date in target_dates:
        ...
        if date in fixed_oos_data["Date Key"].values:
        ...
        elif date >= pd.to_datetime("2025-03-05") and date <= pd.to_datetime("2025-03-31"):
            ...
            if prev_oos_values:
                projected_oos = prev_oos_values[0] * (1 + avg_oos_increase)*1.5  # Apply trend
            else:
    ...
    projected_oos_8mar = np.mean(oos_values)*0.7 if oos_values else 7  # Default to 12 if no valid values
    ...
    for entry in oos_data:
        ...
        if date >= change_date:
            ...
            supply_factor = max(0, min(1, (custom_stl_supply - 40000) / 35000 *0.75 ))
                ...
                #entry["Projected OOS%"] = round(projected_oos_8mar - (7 * days_after_change / 7) * ((supply_factor * 3) + 1), 2)
