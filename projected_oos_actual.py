...
# Define stock thresholds based on the custom supply
...
if supply_file and oos_file:
    ...
    # Use file data only for specific dates
    ...
    for i, date in enumerate(pd.date_range("2025-03-25", "2025-04-30"), start=1):
        ...
        if not historical_oos.empty:
            ...
            # 🔹 **Locked Supply for Specific Dates**
            ...
            # 🔹 **Apply Custom Supply for Other Dates**
            else:
            ...
            # ✅ **OOS Reduction Based on Custom Supply**
            ...
            demand_factor = total_demand / demand_mean

            # ✅ **Final OOS Calculation**
            projected_oos *= demand_factor
            projected_oos = max(0, projected_oos * (1 - i * daily_decrease * (1 + stock_factor)))
