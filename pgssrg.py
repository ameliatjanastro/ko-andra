...
# Load input files
forecast_df = pd.read_excel("forecast pgssrg.excel")  # Product ID, Date, Hub ID, ForecastSTEP 3
hub_wh_map_df = pd.read_csv("hubcontr.csv")  # Hub ID, WH ID
split_sku_df = pd.read_csv("sku srg.csv")  # Product ID column
...
final_df = combined_df.groupby(['Date', 'Product ID', 'Product', 'WH ID'], as_index=False)['ForecastSTEP 3'].sum()
